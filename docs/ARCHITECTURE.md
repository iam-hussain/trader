...
Routes in `src/routes/`. Services (broker, quant client, crypto, signal
engine, risk engine, journal, macro events) in `src/services/`. BullMQ
workers under `src/jobs/` run scheduled briefs (08:00 / 12:00 / 16:30 ET) +
alert evaluation. SSE stream at `/api/sse/events` pushes brief / order /
alert updates to the web app. The API is thin — heavy compute is delegated
to the quant service; persistence goes through `@trader/db` (Prisma).
...
### Redis (port 6379)
BullMQ job queue + ephemeral pub/sub.

**Queues** (`apps/api/src/jobs/`):
- `brief` — generated by the cron scheduler (08:00 / 12:00 / 16:30 ET) and
  by ad-hoc `POST /api/briefs/generate`. Worker calls `generateBrief()`,
  writes the Brief + accepted Signals to Mongo, returns `{ briefId }`.
- `alert` — recurring poll (every 5 min during market hours) that evaluates
  every active alert against the latest data. (Phase 3 ships the worker
  scaffold; real evaluator lands in Phase 3+.)

**Pub/sub channels**: `user:{userId}:events` carries brief / order / alert
updates that the SSE handler at `/api/sse/events` fans out to the browser.

The job system is gated by `JOBS_ENABLED` (default on). Set
`JOBS_ENABLED=false` to run the API without the workers (useful for tests).
